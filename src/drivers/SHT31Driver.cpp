...
SHT31Driver::ErrorCode SHT31Driver::run(float &temperature, float &humidity)
{
    ...
    bool success = i2c::masterTransmitBlocking(i2c_, sht31_address, transmit_data, 2, TIMEOUT_MS);
    if (!success)
    {
    ...
    success = i2c::masterReceiveBlocking(i2c_, sht31_address, receive_data, 6, TIMEOUT_MS);
    if (!success)
    {
        return ErrorCode::Timeout;
    }
